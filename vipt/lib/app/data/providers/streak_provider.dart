...
class StreakProvider implements SqfliteHelper<int, Streak> {
  ...
  /// Batch insert nhiều streaks cùng lúc (nhanh hơn nhiều so với insert từng cái)
  Future<void> batchAdd(List<Streak> streaks) async {
    final db = await DatabaseProvider.database;
    final batch = db!.batch();
    
    for (var streak in streaks) {
      batch.insert(tableName, streak.toMap());
    }
