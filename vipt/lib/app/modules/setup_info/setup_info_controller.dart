...
class SetupInfoController extends GetxController {
  ...
  Future<void> _finishSetupBasicInformation() async {
    ...
    ViPTUser newUser = ViPTUser(
      ...
      currentPlanID: DataService.currentUser?.currentPlanID,
    );
    ...
    if (DataService.currentUser != null) {
      ...
      // Clear tất cả dữ liệu cũ trước khi tạo user mới
      await DataService.instance.clearAllUserData();
      ...
      if (user != null) {
        ...
        // Sau khi tạo user mới, chuyển về Home (bỏ màn hình recommendation preview)
        UIUtils.hideLoadingDialog();
        Get.offAllNamed(Routes.home);
      } else {
        UIUtils.hideLoadingDialog();
  ...
  Future<void> createWorkoutPlan(ViPTUser user) async {
    try {
      ...
      // Sau khi tạo plan trên server, reload user profile để lấy currentPlanID
      try {
        await DataService.instance.loadUserData();
        final planId = DataService.currentUser?.currentPlanID;
        if (planId != null) {
          // Nếu controller đã đăng ký, yêu cầu nó load plan từ server
          if (Get.isRegistered<RecommendationPreviewController>()) {
            final rc = Get.find<RecommendationPreviewController>();
            rc.recommendationData['createdPlanID'] = planId;
            await rc.loadRecommendation();
          } else {
            final rc = Get.put(RecommendationPreviewController(), permanent: true);
            rc.recommendationData['createdPlanID'] = planId;
            await rc.loadRecommendation();
          }
        }
      } catch (e) {
        _log('⚠️ Failed to sync created plan to preview controller: $e');
      }
      ...
      _log('❌ Lỗi khi tạo workout plan từ recommendation: $e');
