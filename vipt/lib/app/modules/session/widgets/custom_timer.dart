...
class MyCircularCountDownTimer extends StatefulWidget {
  ...
  final MyCountDownController? controller;
...
class MyCircularCountDownTimerState extends State<MyCircularCountDownTimer>
  ...
  String get time {
    if (widget.isReverse && _controller!.isDismissed) {
      if (widget.textFormat == MyCountdownTextFormat.MM_SS) {
        return "00:00";
      } else if (widget.textFormat == MyCountdownTextFormat.SS) {
        return "00";
      } else if (widget.textFormat == MyCountdownTextFormat.S) {
        return "0";
      } else {
        return "00:00:00";
      }
  ...
  String _getTime(Duration duration) {
    // For HH:mm:ss format
    if (widget.textFormat == MyCountdownTextFormat.HH_MM_SS) {
      return '${duration.inHours.toString().padLeft(2, '0')}:${(duration.inMinutes % 60).toString().padLeft(2, '0')}:${(duration.inSeconds % 60).toString().padLeft(2, '0')}';
    }
    // For mm:ss format
    else if (widget.textFormat == MyCountdownTextFormat.MM_SS) {
...
/// Controls (i.e Start, Pause, Resume, Restart) the Countdown Timer.
class MyCountDownController {
  late MyCircularCountDownTimerState _state;
